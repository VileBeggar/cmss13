...
/obj/item/device/radio/headset/equipped(mob/living/carbon/human/user, slot)
	...
	if (slot == WEAR_L_EAR || slot == WEAR_R_EAR)
		...
		if(headset_hud_on)
			...
			if(user.mind && (user.assigned_squad || misc_tracking) && user.hud_used && user.hud_used.locate_leader && has_tracker)
...
/obj/item/device/radio/headset/almayer
	...
	item_state = "headset"
	frequency = PUB_FREQ
	has_hud = FALSE //tracker hud is handled by squad helmet visors
	has_tracker = TRUE
