...
/mob/living/carbon/human/verb/holster_verb(unholster_number_offset = 1 as num)
	set name = "holster"
	set hidden = TRUE
	if(usr.is_mob_incapacitated(TRUE) || usr.is_mob_restrained() || IsKnockDown() || (HAS_TRAIT_FROM(src, TRAIT_UNDENSE, LYING_DOWN_TRAIT) && HAS_TRAIT(src, TRAIT_CARRIED_BY_XENO)))
